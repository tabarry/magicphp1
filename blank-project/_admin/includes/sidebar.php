<?php if ($_GET['overlay'] != 1) { ?>
<div class="col-sm-2 sidebar-area" style="padding:0px;">
        <nav>
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                     <td width="10%">
                        &nbsp;
                    </td>
                    <td width="75%" id="sidebar-nav">
                        <div>&nbsp;</div>
            <?php if ($_SESSION[SESSION_PREFIX . 'admin_login'] != '') { ?>
                <?php if ($_SESSION[SESSION_PREFIX . 'user_photo'] != '' && file_exists(ADMIN_UPLOAD_PATH . urldecode($_SESSION[SESSION_PREFIX . 'user_photo']))) { ?>
                    <div class="imgThumbRound" style="background-image: url(<?php echo BASE_URL . 'files/' . urldecode($_SESSION[SESSION_PREFIX . 'user_photo']); ?>);"></div>
                <?php } ?>
                <ul>
                    <li>
                        <a id="lk_home" href="<?php echo ADMIN_URL; ?>index<?php echo PHP_EXTENSION; ?>/home/"><i class="fa fa-home"></i>&nbsp;&nbsp;Home</a>
                    
                    </li>
                    <li><a id="lk_users" href="<?php echo ADMIN_URL; ?>update<?php echo PHP_EXTENSION; ?>/users/<?php echo $_SESSION[SESSION_PREFIX . 'user_id']; ?>/profile/"><i class="fa fa-user"></i>&nbsp;&nbsp;Profile</a></li>
                    <li><a id="lk_themes" href="<?php echo ADMIN_URL; ?>themes<?php echo PHP_EXTENSION; ?>/themes/"><i class="fa fa-picture-o"></i>&nbsp;&nbsp;Themes</a></li>
                    <li class="hr"></li>
                    <?php
                    suBuildFormLinks($viewables);
                    ?>
                    <li class="hr"></li>
                    <li><a href="<?php echo ADMIN_URL; ?>login<?php echo PHP_EXTENSION; ?>/logout/"><i class="fa fa-power-off"></i>&nbsp;&nbsp;Log Out</a>        
                </ul>
            <?php } ?>
            <div>&nbsp;</div>
                    </td>
                    <td width="5%">
                        &nbsp;
                    </td>
                    <td width="10%" id="sidebar-bg">
                        &nbsp;
                    </td>
                </tr>
            </table>
            
        </nav>
    </div>
<?php } ?>
