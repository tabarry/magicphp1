...
//Check if logged in
    function checkAdminLogin() {
        ...
        if ($_SESSION[SESSION_PREFIX . 'admin_login'] == '') {
            $url = ADMIN_URL . 'login' . PHP_EXTENSION . '/?' . suCrypt($_SERVER['PHP_SELF']);
...
if (!function_exists('suCheckGroupPermissions')) {
    ...
    function suCheckGroupPermissions($table) {
        $sql = "SELECT " . suJsonExtract('data', 'add_access') . "," . suJsonExtract('data', 'view_access') . "," . suJsonExtract('data', 'preview_access') . "," . suJsonExtract('data', 'update_access') . "," . suJsonExtract('data', 'delete_access') . "," . suJsonExtract('data', 'duplicate_access') . "," . suJsonExtract('data', 'download_csv_access') . "," . suJsonExtract('data', 'download_pdf_access') . " FROM groups WHERE " . suJsonExtract('data', 'group_title', FALSE) . "='" . $_SESSION[SESSION_PREFIX . 'user_group'] . "' AND " . suJsonExtract('data', 'status', FALSE) . "='Active' AND live='Yes' LIMIT 0,1";
        $result = suQuery($sql);
        //Build add access
        ...
        $groupAccess = array('add_access' => $addAccess, 'view_access' => $viewAccess, 'preview_access' => $previewAccess, 'update_access' => $updateAccess, 'delete_access' => $deleteAccess, 'duplicate_access' => $duplicateAccess, 'download_csv_access' => $downloadAccessCSV, 'download_pdf_access' => $downloadAccessPDF, 'viewables' => $viewables, 'previewables' => $previewables, 'addables' => $addables, 'updateables' => $updateables, 'deleteables' => $deleteables,);
        return $groupAccess;
    }
