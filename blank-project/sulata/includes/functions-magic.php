        ...
        if ($numRows > 0) {
            ...
            foreach ($row as $value) {
                ...
                if ($_SESSION[SESSION_PREFIX . 'user_group'] == ADMIN_GROUP_NAME) {
                    echo "<li><a id=\"lk_" . suUnstrip($value['slug']) . "\" href=\"" . ADMIN_URL . "manage" . PHP_EXTENSION . "/" . suUnstrip($value['slug']) . "/\"><i class=\"fa fa-angle-double-right\"></i>&nbsp;&nbsp;" . suUnstrip($value['title']) . "</a></li>";
                } else {
                    if (in_array($value['slug'], $viewables)) {
                        echo "<li><a id=\"lk_" . suUnstrip($value['slug']) . "\" href=\"" . ADMIN_URL . "manage" . PHP_EXTENSION . "/" . suUnstrip($value['slug']) . "/\"><i class=\"fa fa-angle-double-right\"></i>&nbsp;&nbsp;" . suUnstrip($value['title']) . "</a></li>";
                    }
                }
            }
        ...
        switch ($arr['Type']) {
            ...
            case "textbox":
                ...
                if ($labelRequirement == 'Yes') {
                    ...
                    if ($getSettings['show_clear_field'] == 1) {
                        $clearIcon = "<a href='javascript:;' onclick=\"$('#" . $arr['Slug'] . "').val('')\"><i class='fa fa-times-circle-o'></i></a>";
                    } else {
                        $clearIcon = "";
                    }
            ...
            case "picture_field":
                ...
                if ($labelRequirement == 'Yes') {
                    ...
                    echo "<label id='lbl_" . $arr['Slug'] . "'>" . $requiredStar . $arr['Name'] . ":</label> <sup>" . $allowed . "</sup>";
                } else {
                    $arg = array_merge($arg, array('placeholder' => $requiredStar . $arr['Name']));
                }
                //Build title
...
//                            $arg = array_merge($arg, array('required' => 'required'));
            ...
            case "currency":
                ...
                if ($labelRequirement == 'Yes') {
                    ...
                    echo "<label id='lbl_" . $arr['Slug'] . "'>" . $requiredStar . $arr['Name'] . ": {$clearIcon}</label> <sup>" . $getSettings['site_currency'] . "</sup>";
                } else {
